...
$soocool_action_scheduler_runtime_file = __DIR__ . '/src/Infrastructure/ActionSchedulerRuntime.php';
...
if ( is_readable( $soocool_action_scheduler_runtime_file ) && is_readable( $soocool_email_labels_file ) && is_readable( $soocool_order_actions_file ) ) {
	require_once $soocool_action_scheduler_runtime_file;
...
delete_option( 'soocool_settings_write_lock' );
...
delete_option( 'soocool_auto_submit_migration_20260811' );
...
// Remove leftover per-order sync locks, API cooldowns, manual API-test results, webhook replay transients, and label transients.
...
$wpdb->query(
	$wpdb->prepare(
		"DELETE FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s OR option_name LIKE %s",
		...
		$wpdb->esc_like( '_transient_soocool_api_cooldown_' ) . '%',
		$wpdb->esc_like( '_transient_timeout_soocool_api_cooldown_' ) . '%',
